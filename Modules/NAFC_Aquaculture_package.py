...
This package will include all NAFC-Aquaculture QA/QC functions.

'''

'''
TESTING AREA
input_path = '/home/joc000/gpfs7/Data/CASTS/Data_Input/NAFC_Aquaculture/data_raw/'
max_depth=5000
'''
...
def raw_to_netcdf(
	...
	for folder in folders:
		...
		for file in files:
			...
			data = pd.read_csv(input_path + folder +'/'+ file, header=0, delimiter=r"\s+").to_dict('list')
			...
			nc_out = nc.Dataset(input_path+'individual_nc/'+file[:-4]+'.nc','w')
		
			#File information
			nc_out.Conventions = 'CF-1.6' #Ask Fred what this means
			nc_out.title = 'NAFC_aquaculture, Netcdf File' #Temporary title for the .nc file
...
def netcdf_gather(
	...
	'''
	file_input: path location to individual netcdf files
	file_output: path location for output yearly files
	years: years that data is from (array)
	'''

	#Now go about joining all like-year nc files together
	files = os.listdir(file_input+'individual_nc/')
	...
	for year in years:
		...
		for file in files:
			if file[:4] == str(year):
				nc_files.append(file_input+'individual_nc/'+file)
		...
		nc_out.to_netcdf(file_output+str(year)+'.nc',encoding={'time':{'units': "seconds since 1900-01-01 00:00:00"}})
		nc_out.close()
