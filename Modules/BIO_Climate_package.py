...
This package will include all BIO Climate QA/QC functions.

'''

'''
TESTING AREA
file_input = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/BIO_Climate/R_Files/'
file_name = 'BIO_Climate_Databases'
file_output = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/BIO_Climate/NetCDF/'
max_depth=5000
'''

def rdata_to_netcdf_pre2010(
	file_input,
	file_name,
	file_output,
	max_depth=5000,
	...
	rfile = pyreadr.read_r(file_input)
	...
	for year in years[:]:
		...
		#Set up the .nc file
		nc_out = nc.Dataset(file_output+file_name+'/'+str(year)+'_temporary.nc','w')
	...
	for year in years:
		...
		#Open the dataset
		ds = xr.open_dataset(file_output+file_name+'/'+str(year)+'_temporary.nc')
		...
		if ds.time.size > 0:
			ds.to_netcdf(file_output+file_name+'/'+str(year)+'.nc','w')
		ds.close()

		#Remove the temporary file
		expr = 'rm '+file_output+file_name+'/'+str(year)+'_temporary.nc'
		os.system(expr)
		print(str(year)+' done.')


def rdata_to_netcdf_post2010(
	file_input,
	file_name,
	file_output,
	max_depth=5000,
	...
	for year in years:
		...
		cruise_id = ds.Cruiseid.values.astype(str)
		...
		ds = ds.sel(time = ds.datatype != 'GL') #For 2018
