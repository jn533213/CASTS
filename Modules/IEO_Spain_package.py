...
This package will include all IEO-Spain QA/QC functions.

'''

'''
TESTING AREA
file_input = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/IEO_Spain/data_raw/'
file_output = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/IEO_Spain/data_processed/'
years = np.arange(1993,2023+1).astype(str)
max_depth = 5000
'''
...
def raw_to_netcdf(
	...
	files = np.array(os.listdir(file_input))
	...
	for file in files:
		...
		with open(file_input+file, encoding='ISO-8859-1') as line:
		...
		#Set up the .nc file
		nc_out = nc.Dataset(file_output+file[:-4]+'.nc','w')
...
'''
TESTING AREA
file_input = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/IEO_Spain/data_processed/'
file_output = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/IEO_Spain/netcdf_yearly/'
max_depth = 5000
'''
...
def netcdf_yearly(
	...
	files = np.array(os.listdir(file_input))
	files = files[files != 'nc_old']

	#Now cycle through all the NetCDF files and save by year
	ds = {}
	for file in files:
		ds[file[:-3]] = xr.open_dataset(file_input+file)

	#Merge all of the sources together
	ds_merged = xr.concat([ds[file[:-3]] for file in files],dim='time',combine_attrs='override')
...
def EU_NAFO_txttonetcdf(
	file_input='/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/IEO_Spain/data_raw/NAFO_EU/',
	file_output='/home/joc000/gpfs7/Data/CASTS/Data_Input/IEO_Spain/netcdf_yearly/EU_NAFO/',
	max_depth=5000,
	):
	...
	#Manually define instrument ID
	instrument_ids = nc_out.createVariable('instrument_id',str,('time'),zlib=True)
	instrument_ids[:] = np.full(pd_1d['Time'].size,'CTD')
