...
This package will include all Polar Data Catalogue QA/QC functions.
...
'''
TESTING AREA
file_input = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/Polar_Data_Catalogue/data_raw/'
file_output = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/Polar_Data_Catalogue/data_product/netcdf_group/'
file_yearly_output = '/gpfs/fs7/dfo/dpnm/joc000/Data/CASTS/Data_Input/Polar_Data_Catalogue/data_product/netcdf_yearly/'
max_level = 5000
'''
...
def raw_to_netcdf(
	...
	folders = next(os.walk(file_input))[1]
	...
	for folder in np.sort(folders):
		...
		cast_files = []
		for paths, subdirs, files in os.walk(file_input+folder):
			for name in files:
		...
		for i,value in enumerate(cast_files):
			if value[-3:] == 'int':
				filt[i] = True
			if folder.startswith('CCIN12713') or folder == '2023':
				if value[-3:] == 'csv':
					filt[i] = True
			if value[-8:] == 'info.int':
				filt[i] = False
		...
		temperature = np.zeros((cast_files.size, max_level)).astype(float)
		salinity = np.zeros((cast_files.size, max_level)).astype(float)
		density = np.zeros((cast_files.size, max_level)).astype(float)
		...
		for i,cast_file in enumerate(cast_files):
			...
			if cast_file[-3:] == 'csv':
				...
				#Determine the csv meta data
				cast_head = [line.split() for line in open(cast_file)]
				#Determine where the start of the data is
				for ii,value in enumerate(cast_head):
					if value[0] != '%':
						header_end = ii
						break
				cast_head = cast_head[:header_end]

				#Cycle through the header data and isolate meta data
				for ii in cast_head:
					if np.size(ii) > 1:
						if ii[1] == 'Cruise_Number:':
							cruise_number[i] = ii[2]
						if ii[1] == 'Cruise_Name:':
							cruise_name[i] = '_'.join(ii[2:])
						if ii[1] == 'Original_Filename:':
							file_name[i] = folder+'__'+ii[2]
						if ii[1] == 'Start_Date_Time':
							time_1D[i] = np.datetime64(ii[3])
						if ii[1] == 'Initial_Latitude':
							latitude[i] = float(ii[3])
						if ii[1] == 'Initial_Longitude':
							longitude[i] = float(ii[3])

				#Import the csv file data
				cast_data = pd.read_csv(cast_file, header=header_end)

				#Record the 2D data
				temperature[i,:] = stats.binned_statistic(
					cast_data['PRES'].values,
					cast_data['TE90'].values,
					'mean',
					bins=np.arange(max_level+1)
					).statistic
				salinity[i,:] = stats.binned_statistic(
					cast_data['PRES'].values,
					cast_data['PSAL'].values,
					'mean',
					bins=np.arange(max_level+1)
					).statistic
				density[i,:] = stats.binned_statistic(
					cast_data['PRES'].values,
					cast_data['SIGT'].values,
					'mean',
					bins=np.arange(max_level+1)
					).statistic
		...
		#Set up the .nc file
		nc_out = nc.Dataset(file_output+folder+'.nc','w')
...
def yearly_netcdf(
	...
	for folder in folders:
		ds[folder] = xr.open_dataset(file_output+folder+'.nc')
	...
	for year in years:
		...
		ds_isolate.to_netcdf(file_yearly_output+str(year)+'.nc',\
